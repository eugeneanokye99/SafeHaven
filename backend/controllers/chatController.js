...
const { getWebResponse } = require('../utils/webScraper');
...
async function getResponse(message) {
  ...
  let response = decisionTreeSearch(normalizedMessage);

  return response;
}

async function decisionTreeSearch(query) {
  const nodes = await Node.find();
  let currentNode = nodes.find(node => node.id === 'root');

  while (currentNode) {
    if (currentNode.options) {
      let foundOption = false;
      for (const option of currentNode.options) {
        if (query.toLowerCase().includes(option.text.toLowerCase())) {
          if (option.response) {
            return option.response;
          } else {
            currentNode = nodes.find(node => node.id === option.next_node);
            foundOption = true;
            break;
          }
        }
  ...
  // If no suitable response is found, call the web scraping function
  const webScrapingResponse = await getWebResponse(query);
  return webScrapingResponse || "I'm sorry, I couldn't find an answer to your question. Please try rephrasing or ask a different question.";
}
